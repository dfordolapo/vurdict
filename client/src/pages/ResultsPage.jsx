...
export default function ResultsPage() {
  ...
  const handleDownload = () => {
    ...
    dims.forEach((d) => {
      ...
      if (cat) {
        dimSection += `  ${d.label.padEnd(22)} ${String(cat.score).padStart(3)}/100  ${cat.explanation}\n`;
      }
    });
    ...
    const content = [
      ...
      `════════════════════════════════════════════════════════`,
      ``,
      `  Analyzed URL:  ${state.url || 'N/A'}`,
      `  Date:          ${formattedDateTime}`,
      `  Goal:          ${state.goal === 'win_clients' ? 'Win Freelance Clients' : 'Get Hired'}`,
      `  Experience:    ${state.experience || 'Junior'}`,
      ``,
      line,
      ...
      `  ── TOP RECOMMENDATION ──`,
      ``,
      `  ${report.fix_this_first?.title || 'N/A'}`,
      `  ${report.fix_this_first?.description || ''}`,
      `  Priority: ${report.fix_this_first?.priority_score || '-'}/10`,
      ``,
      line,
      `  Generated by Vurdict — AI Portfolio Feedback`,
      `  https://vurdict.vercel.app`,
    ].join('\n');
    ...
    a.href = url;
    a.download = `vurdict-report-${Date.now()}.txt`;
